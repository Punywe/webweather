...
# ── GET: ดึงรายการ node ทั้งหมด (admin ใช้) ───────────────────────────────
@router.get("/")
...
async def update_node(node_id: int, data: DataNode):
    ...
    try:
        ...
        if cursor.rowcount == 0:
            raise HTTPException(status_code=404, detail="ไม่พบ node ที่ต้องการแก้ไข")
        conn.commit()
...
async def delete_node(node_id: int):
    ...
    try:
        ...
        if cursor.rowcount == 0:
            raise HTTPException(status_code=404, detail="ไม่พบ node ที่ต้องการลบ")
        conn.commit()
